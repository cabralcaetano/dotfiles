...
#
# Mechanism: the active super workspace number is persisted in $STATE_FILE.
# SUPER+N and SUPER+SHIFT+N binds call this script instead of dispatching a raw
# workspace number, so they always resolve to name:super-<sw>-<n>. The Hyprland
# internal name is only a collision-free target; notifications and waybar expose
# only the numeric super workspace and its icon.
#
...
#   super-workspace.sh next|prev            cycle active super workspace, jump to slot 1
...
case "$cmd" in
  ...
  next|prev)
    ...
    hyprctl dispatch "hl.dsp.focus({ workspace = \"name:$(workspace_name "$NEW" 1)\" })" >/dev/null
    sync_waybar "$NEW"
